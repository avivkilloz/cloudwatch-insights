...
def list_saved_sessions(page: Optional[str] = None, db: Session = Depends(get_db)):
    query = db.query(models.SavedSession)
    if page:
...
def create_saved_session(payload: schemas.SavedSessionCreate, db: Session = Depends(get_db)):
    saved = models.SavedSession(**payload.model_dump())
    db.add(saved)
    db.commit()
    db.refresh(saved)
    return saved
...
def update_saved_session(saved_session_id: int, payload: schemas.SavedSessionUpdate, db: Session = Depends(get_db)):
...
def delete_saved_session(saved_session_id: int, db: Session = Depends(get_db)):
    saved = db.get(models.SavedSession, saved_session_id)
    if not saved:
        raise HTTPException(status_code=404, detail="Saved session not found")
    db.delete(saved)
